...
class PrecomputedCandidateStore:
    ...
    def __init__(
        ...
        *,
        ...
        if self.manifest is not None:
            ...
            self.targets = self._normalize_targets(
                self.targets, schema_version=self.manifest.schema_version
            )
            self._validate_targets(
                self.targets,
                allow_null_timestamp=(
                    self.manifest.schema_version == LEGACY_ARTIFACT_SCHEMA_VERSION
                ),
            )
            self.targets["user_id"] = self.targets["user_id"].astype(str)
            self.targets["target_item_id"] = self.targets["target_item_id"].astype(str)
            self.targets = self.targets.sort_values(
                ["user_id", "target_order", "target_item_id"], kind="mergesort"
    ...
    def _normalize_targets(
        frame: pd.DataFrame, *, schema_version: str
    ) -> pd.DataFrame:
        observed_columns = tuple(frame.columns)
        if schema_version == LEGACY_ARTIFACT_SCHEMA_VERSION:
            if observed_columns != LEGACY_TARGET_COLUMNS:
                raise ValueError(
                    "Legacy target artifact schema/order mismatch: "
                    f"{observed_columns!r} != {LEGACY_TARGET_COLUMNS!r}"
                )
            promoted = frame.copy()
            promoted.insert(2, "target_order", 1)
            promoted.insert(3, "target_timestamp", np.nan)
            uncovered = ~promoted["target_model_covered"].astype(bool)
            promoted.loc[
                uncovered,
                ["target_raw_score", "target_base_score", "target_full_rank"],
            ] = np.nan
            return promoted.loc[:, TARGET_COLUMNS]
        return frame

    @staticmethod
    def _validate_targets(
        frame: pd.DataFrame, *, allow_null_timestamp: bool = False
    ) -> None:
        observed_columns = tuple(frame.columns)
        ...
        if frame.astype({"user_id": str, "target_item_id": str}).duplicated(
            ["user_id", "target_item_id"]
        ).any():
            raise ValueError("Target artifact contains duplicate user/target pairs")
        orders = pd.to_numeric(frame["target_order"], errors="coerce")
        if orders.isna().any() or (orders < 1).any() or (orders % 1 != 0).any():
            raise ValueError("target_order must contain positive integers")
        for user_id, group in frame.assign(_order=orders.astype(int)).groupby("user_id"):
            if sorted(group["_order"].tolist()) != list(range(1, len(group) + 1)):
                raise ValueError(f"Non-contiguous target_order for user {user_id}")
            timestamps = pd.to_numeric(group["target_timestamp"], errors="coerce")
            if not allow_null_timestamp and not np.isfinite(
                timestamps.to_numpy(dtype=float)
            ).all():
                raise ValueError("v2 target_timestamp values must be finite")
            ordered_timestamps = timestamps.iloc[np.argsort(group["_order"].to_numpy())]
            finite_timestamps = ordered_timestamps.dropna().to_numpy(dtype=float)
            if (np.diff(finite_timestamps) < 0).any():
                raise ValueError(f"target_timestamp order is not temporal for user {user_id}")
        ...
        covered = coverage.to_numpy(dtype=bool)
        if not np.isfinite(scores.loc[covered].to_numpy(dtype=float)).all():
            raise ValueError("Covered targets must have finite scores")
        covered_base = scores.loc[covered, "target_base_score"].to_numpy(dtype=float)
        if ((covered_base < 0.0) | (covered_base > 1.0)).any():
            raise ValueError("target_base_score must lie in [0, 1]")
        covered_ranks = ranks.loc[covered]
        if (
            covered_ranks.isna().any()
            or (covered_ranks < 1).any()
            or (covered_ranks % 1 != 0).any()
        ):
            raise ValueError("Covered target_full_rank values must be positive integers")
        uncovered = ~covered
        if scores.loc[uncovered].notna().any().any() or ranks.loc[uncovered].notna().any():
            raise ValueError("Model-uncovered targets must have null scores and rank")
    ...
    def _validate_manifest_metadata(self) -> None:
        ...
        if self.targets is not None and set(self.targets["user_id"]) != set(self._groups):
        ...
        if self.targets is not None:
            for target in self.targets.to_dict("records"):
                ...
                if not bool(target["target_model_covered"]):
                    if not matches.empty:
                        raise ValueError(
                            f"Model-uncovered target {target_id} unexpectedly appears for {user_id}"
                        )
                    continue
    ...
    def target_row(self, user_id: str) -> Dict[str, Any]:
        rows = self.target_rows(user_id)
        if len(rows) != 1:
            raise ValueError(
                f"User {user_id} has {len(rows)} positives; use target_rows/target_records"
            )
        return rows[0]

    def target_rows(self, user_id: str) -> list[Dict[str, Any]]:
        if self.targets is None:
            raise RuntimeError("A manifest with a target artifact is required")
        user_id = str(user_id)
        rows = self.targets[self.targets["user_id"] == user_id]
        if rows.empty:
    ...
    def target_record(self, user_id: str) -> CandidateRecord:
        ...
        return self._target_record_from_row(row)

    def target_records(self, user_id: str) -> list[CandidateRecord]:
        return [self._target_record_from_row(row) for row in self.target_rows(user_id)]

    def scored_target_records(self, user_id: str) -> list[CandidateRecord]:
        """Return only model-covered positives that have auditable model scores."""

        return [
            self._target_record_from_row(row)
            for row in self.target_rows(user_id)
            if bool(row["target_model_covered"])
        ]

    def _target_record_from_row(self, row: Mapping[str, Any]) -> CandidateRecord:
        item_id = str(row["target_item_id"])
        if not bool(row["target_model_covered"]):
            raise ValueError(f"Model-uncovered target {item_id} has no model score")
        if self._catalog is None:
            metadata: Dict[str, Any] = {}
        else:
            if item_id not in self._catalog.index:
        ...
        metadata.update(
            {
                ...
                "target_timestamp": (
                    ...
                    else float(row["target_timestamp"])
                ),
            }
        )
        metadata = {
