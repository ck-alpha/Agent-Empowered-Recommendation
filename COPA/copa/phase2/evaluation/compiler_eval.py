...
def _constraint_set(constraints: Iterable[Mapping[str, Any]]) -> set[Tuple[str, str, str]]:
    return {
        (str(item["attribute"]), str(item["operator"]), _freeze_value(item.get("value")))
...
def _expected_constraints(
    ...
    return plan.executable_constraints
...
def evaluate_compiler(
    ...
    for case in cases:
        ...
        if case["expected_status"] == "success":
            expected_specs = _expected_constraints(case, domain, candidates)
